...
@ 원본은 살펴볼 마디가 하나도 없으면 있지도 않은 해를 센다. 마디 없는 그래프를
주든, 선택항 \.{n0}이나 \.{n-1}로 마디를 모두 잘라내든, 원본은 ``Altogether 999
solutions, 999 nodes''라고 알리고 멀쩡히 끝난다. 이 판은 마디가 너무 많을 때처럼
거절하고 끝낸다. 이 결함은 {\mc SSHAM}의 후손인 {\mc SSBIDIHAM}을 옮기다가 찾았다.
그쪽 원본은 같은 자리에서 아예 죽는다.
...
@ 원본과 다른 곳이 다섯 군데 있다. 넷은 알고리즘이 아니라 언어에서 온 것이고,
마지막 하나는 원본의 결함을 고친 것이다.
...
다섯째, 살펴볼 마디가 없으면 거절한다. 원본은 이때 없는 해를 $999$개 센다. 그래프를
읽어 들이는 자리에서 이미 말했다. 이 경우만은 원본과 출력이 다르다.
