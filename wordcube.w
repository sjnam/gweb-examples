...
미리 답을 말하면 $83{,}576$개다. 그 가운데, 여는 그림처럼 열다섯 줄이 {\it 모두 서로
다른\/} 낱말인 것만 세면 $75{,}130$개로 줄어든다. 답에 이르는 백트래킹은 놀랄 만큼
단순한데, 거기에 {\it 선행 배제\/}(preclusion)라는 전방 검사를 곁들여 탐색 나무를
크게 줄인다(마지막 앞 장).

@ 표는 표일 뿐이니 실물도 한번 보자. 그림 1의 낱말들을 글자로 풀어 $125$칸에 채워
...
\pic{wordcube-2.pdf}
$$
\figcap{그림 2: 같은 정육면체를 등각 투영으로 본 모습. 오른쪽 면에서 파란 줄은
아래에서 위로 읽어 |extra|이고, 초록 줄은 오른쪽 위에서 왼쪽 아래로 읽어
|strap|이다. 둘이 만나는 칸을 비우고, 그 자리에서 속으로 뻗은 셋째 방향의 낱말
|adopt|$(=C_{12\ast})$를 글자 하나씩 떼어 오른쪽으로 뽑아냈다. 빈칸에 있던 글자
