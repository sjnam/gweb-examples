...
이 $5\times5\times5$ 격자의 $125$칸을 글자로 채우되, 세 축 가운데 어느 방향으로 다섯
...
\endmplibcode
$$
\figcap{{\bf 그림 1}: 대칭 단어 정육면체 하나를 $5\times5$ 낱말 표로 편 모습. 여기서 $(i,j)$칸은
...
아니라 한 겹 더 깊은 대칭이 숨어 있다. 곧 $(i,j)$낱말의 $k$번째 글자는 $(i,k)$낱말의
...
정육면체는 저마다 딱 한 번씩 세어진다. 그러니 $83{,}576$은 있는 그대로의 개수다.
...
\smallskip
{\narrower\noindent{\bf 채우기 불변식.} 칸 $(i,j)$낱말을 놓을 차례가 되면, 그 낱말의
앞 $j$글자(위치 $0,\ldots,j-1$)는 이미 정해져 있고, 뒤 $n-j$글자(위치 $j,\ldots,n-1$)는
아직 어느 칸에도 놓인 적 없는 새 글자다.\par}
\smallskip\noindent
왜 그런가. 칸 $(i,j)$낱말의 위치 $c$는 다중집합 $\{i,j,c\}$다. 이 칸은 자신을 이루는
...
@* 백트래킹.
탐색의 독립 단위는 대각 낱말 $a_{00\ast}$ 하나를 고르는 일이다. 함수 |searchFrom(w)|은 그것을
...
@ 이제 심장이다. 함수 |solve(t)|는 |order|의 $t$번째 낱말을 놓는다. 차례가 $t=15$에 이르면
