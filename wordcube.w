...
부른다. 여기에 차원을 하나 더 얹은 것이 이 글의 주인공, {\it 단어 큐브\/}다.
...
큐브가 그대로이므로, 세 방향의 낱말들이 실은 한 벌이다. 말보다 그림이다. 아래는
그런 큐브 하나를, 셋째 축을 읽기 방향으로 삼아 $(i,j)$칸마다 낱말 $C_{ij\ast}$를
적어 펼친 것이다.

\medskip
\centerline{\pic{wordcube-1.pdf}}
\figcap{그림 1: 대칭 단어 큐브 하나를 $5\times5$ 낱말 표로 편 모습. $(i,j)$칸은
...
큐브는 저마다 딱 한 번씩 세어진다. $83576$은 있는 그대로의 개수다.
...
@* 대칭과 채우는 순서.
대칭 큐브는 첨자를 정렬한 $i\le j\le k$짜리 칸, 곧 크기 $3$짜리 다중집합
...
큐브 하나가 온전히 채워진 것이니 한 개를 세고 돌아온다. 그렇지 않으면 채우기
...
그중 극히 일부만 큐브로 자라 오른다.

@<결과를 보고한다@>=
	fmt.Printf("대칭 단어 큐브는 모두 %d개다", count)
