...
$S$의 마디가 $m$개, $T$의 마디가 $n$개일 때 실행 시간은 최악의 경우에도
...
$S$의 뿌리는 차수가~$1$이라고 가정한다. 그러니 뿌리이면서 잎이기도 하고,
...
번호). |deg|와 |arc|는 $S$에서는 쓰이지 않고 $T$에서만 필요하다. 같은 마디의
...
하나와 그 반대 하나다. $u$에서 $v$로 가는 호에는 $0$부터 $2n-3$까지의 정수를
...
@ 여기가 원본에서 딱 한 군데 손본 곳이다. 크누스는 남은 문턱을
$$\hbox{\.{for (maxdeg=d-1,emax=e;d<m;d++) o,thresh[d]=emax;}}$$
로 채운다. 곧 차수가 $m-1$인 자리까지다. 그런데 |solve|는 |thresh[n+1]|을 |n|이
...
@ 최대 차수가 $61$이면 한 술 더 뜬다. |thresh|는 |maxn|칸이라 첨자가 $0$부터
$61$까지인데 |thresh[62]|를 짚는다. 배열 밖이다. \CEE/ 표준으로는 무슨 일이
일어나도 좋은 자리이고, 무엇이 읽힐지는 링커가 그 주소에 무엇을 놓았느냐에
달린다. 내가 컴파일해 주소를 찍어 보니 마침 |tip[0]|과 겹쳤다. |tip|은 첨자를
언제나 $1$부터 쓰므로 그 칸은 끝까지 $0$인 채로 남고, 그래서 프로그램은 아무 일
없다는 듯 지나간다. \GO/에서는 그 자리에서 죽는다.

@ 다행히 답이 틀어지지는 않는다. 읽히는 값이 |e|와 같아야 반복문이 |n|을 잘못
올릴 텐데, |e|는 결코 그 값이 될 수 없기 때문이다. |e|는 |thresh[r+1]|에서
시작하고 여기서 $r\ge1$이며, 나무에는 잎이 적어도 둘 있으니 |thresh[2]|는 $2$
이상이다. 그러니 |e|도 $2$ 이상이라 $0$이나 $1$ 따위와 마주칠 일이 없다.

그래도 기대고 있는 것이 배치의 운인 것은 마찬가지다. 여기서는 배열을 한 칸 늘리고
|maxdeg+1|까지 반드시 채워, 읽히는 값이 뜻대로 |emax|가 되게 했다. (원본이 이
대목을 밟는 입력은 어렵지 않게 만들 수 있다. 이를테면 $S=\.{.01}$이고 $T$가 별
모양으로 마디 $62$개일 때다.)
...
@ $u$에서 $v$로 가는 호에는 쌍대가 있다. $v$에서 $u$로 가는 호다. 부모로 가는
...
마디이고, 둘째 첨자~|e|는 $T$의 호다. |e|가 $u$에서 $v$로 가는 호라 하고,
$u$에 뿌리를 두면서 $v$를 품는 $T$의 부분나무를 생각하자. 그것을
``부분나무~|e|''라 부르기로 한다. |p|에 뿌리를 둔 $S$의 부분나무를, |p|를 $v$에
...
$p$에게 아들이 $r$명 $q_1$, \dots, $q_r$ 있고, $v$에게 이웃이 $s+1$명 $u_0$,
...
|p|의 자손 |d| 가운데 심을 수 없는 것이 있으면 |solve|는 $-d$를 돌려준다.
...
\.{hopcroft-karp.w}에 다 있으니 여기서는 되풀이하지 않는다. |p|의 아들들이 그
...
|solx[p]|$[e\dts e+n-1]$에는 마지막 |imate| 표를 넣고, |soly[p]|에는 |g|에서
...
$S$의 부분나무와 그 부모를 $T$에 동형으로 심을 수 없다는 뜻이다. $0$이면 마디~$1$의
...
마지막 줄이 답이다. $S$의 마디 $0$, $1$, \dots, $18$이 차례로 $T$의 어느 마디로
가는지를 적은 것이다.
...
@ 무작위로 나무 짝 $1100$개를 지어 원본과 견주었다. 답도, 찍히는 심기도, mem 수도
모두 같았다. 다만 문턱 배열을 손본 자리에서 입력 mem이 하나 더 든 경우가 있는데,
그것은 정확히 원본이 그 칸을 비워 둔 채 읽던 경우---$T$의 최대 차수가 $S$의 마디
수 이상일 때---뿐이다. 위의 예제는 최대 차수가 $7$이고 $S$의 마디가 $19$개이니
거기에 들지 않아, 수가 그대로 맞는다.
