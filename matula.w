...
잘못 올릴 텐데, |e|는 결코 $0$이 될 수 없기 때문이다. |e|는 |thresh[r+1]|에서
시작하고 여기서 $r\ge1$이며, 나무에는 잎이 적어도 둘 있으니 |thresh[2]|는 $2$
이상이다.

그러나 지켜 주는 것이 이웃 칸의 값일 뿐이라는 것도 분명하다. |thresh| 바로 뒤에
...
그런데 이 글을 쓰는 사이에 크누스가 사이트의 파일을 갈았다. \.{Last-Modified}가
...
@ 무작위로 나무 짝 $1100$개를 지어 원본과 견주었다. $S$는 마디 $2$개에서 $30$개까지,
$T$는 $62$개까지, 가지 뻗는 정도도 여러 가지로 두었다. 답도, 찍히는 심기도, mem
수도 모두 같았다. 예외는 하나도 없다.

문턱 배열을 고치기 전의 판과 견줄 때는 어긋나는 자리가 하나 있었다. $T$의 최대
