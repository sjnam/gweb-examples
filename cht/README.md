...
[«정저지와»](frog/frog.pdf)는 우물을 나선 개구리와 함께 기법을 처음부터
...
[«과하탁교»](bridge/bridge.pdf)는 그 단조성이 둘 다 깨진 문제다 --- 다리를
...
[«선견지명»](cash/cash.pdf)에서 두 모음이 합류한다 --- 미래 시세를 다 아는
...
[«군웅할거»](segment/segment.pdf)는 그 선견지명을 압수당한 문제다 --- 좌표가
...
각 디렉터리의 `*.go`는 `gtangle`이 `.w`에서 뽑은 생성물이고
`*.pdf`는 `gweave`와 luatex가 조판한 문서다 --- 손대지 말 것. 원본은 `.w`와
그림(`*.mp`)뿐이다.

## 빌드

[GWEB](https://github.com/sjnam/gweb)(`gtangle`, `gweave`)과 TeX Live
(luatex, ko.TeX, MetaPost), Go가 필요하다. 프로그램만 돌릴 거라면 생성물이
이미 들어 있으니 Go만 있으면 된다.
