...
@ 줄마다 뜻이 있다. \.{strong component}~$v$는 새 성분이 하나 완성됐고 그 우두머리가
$v$라는 말이다. 우두머리는 그 성분에서 깊이 우선 번호가 가장 작은 꼭짓점, 곧 탐색이
그 성분에 처음 발을 들일 때 밟은 꼭짓점이다.

\.{tree}와 \.{inner}는 성분 {\it 안\/}의 화살이다. \.{tree}는 나무 화살이고,
...
NP-완전이다. 하지만 군더더기는 거의 없고, 무엇보다 공짜로 얻어진다.

\.{link $X$ to $Y$: $u$->$v$}는 성분 $X$에서 성분 $Y$로 건너가는 화살이 있다는
...
핵심은 각 꼭짓점 $v$에 딸린 값 $\.{LOW}(v)$다. 정의는 이렇다: $v$의 나무 아래에
있는 어떤 꼭짓점에서 화살 하나를 타고 갈 수 있는 모든 곳 --- 다만 이미 성분이
확정된 곳은 빼고 --- 그리고 $v$ 자신, 이들의 $p$ 값 중 가장 작은 것.

$$\mplibcode fig_low; \endmplibcode$$
\figcap{그림 2. 같은 그래프의 깊이 우선 나무. 괄호 안은 $(p,\.{LOW})$다. 점선은
나무 아닌 화살. $\.{LOW}(v)=p(v)$인 세 꼭짓점 $f,d,b$가 곧 성분의 우두머리다.}

@ 그림~2를 보면 알고리즘의 전부가 한눈에 들어온다. $\.{LOW}(v)=p(v)$인 꼭짓점이
성분의 우두머리이고, 같은 $\.{LOW}$ 값을 가진 것들이 한 성분을 이룬다. 왜 그런가?

$\.{LOW}(v)<p(v)$라면 $v$의 나무 아래 어딘가에서 $v$보다 먼저 밟힌 꼭짓점 $w$로
가는 화살이 있다는 말이다. 그 $w$는 반드시 $v$의 조상이다(깊이 우선 탐색에서 이미
밟혔고 아직 성분이 확정되지 않은 꼭짓점은 조상이거나 조상의 아래에 있는데, 뒤엣것
이면 $w$의 성분이 이미 끝났을 것이므로). 그러니 $v$는 조상까지 되돌아갈 수 있고,
조상은 나무를 타고 $v$로 내려올 수 있다. 둘은 한 성분이다.

거꾸로 $\.{LOW}(v)=p(v)$라면 $v$의 나무 아래에서는 $v$ 위로 한 발짝도 못 올라간다.
$v$의 성분은 $v$의 나무 아래에 온전히 갇혀 있고, $v$가 그 성분에서 가장 먼저 밟힌
꼭짓점이다.

@ 그래서 알고리즘은 이렇게 굴러간다. 깊이 우선으로 내려가면서 번호를 매기고,
올라오면서 $\.{LOW}$를 부모에게 물려준다. 어떤 꼭짓점 $v$의 탐색이 끝났을 때
$\.{LOW}(v)=p(v)$이면 성분 하나가 완성된 것이니, 아직 성분이 정해지지 않은
꼭짓점들 가운데 $v$ 이후에 밟힌 것들을 모두 $v$의 성분으로 거두어들인다.

``아직 성분이 정해지지 않은 꼭짓점들''을 담아 두는 곳이 필요하다. 타잔의 원래
...
|par|는 깊이 우선 나무에서의 부모다. 아직 밟지 않은 꼭짓점은 |null|이고, 나무의
뿌리는 파수꾼 |sent|를 가리킨다. 그러니 이 밭 하나가 ``밟았는가''와 ``어디서 왔는가''를
겸한다.

|link|는 가장 바쁘다. $v$의 탐색이 도는 동안에는 $\.{LOW}(v)$를 낮춰 준 화살의
끝점을 가리킨다(아무도 낮추지 않았으면 |sent|). 탐색이 끝나 |sink| 웅덩이로
들어가면 스택의 다음 칸을 가리키고, 성분이 확정돼 |settled| 목록으로 옮겨 가면
거기서 또 다음 칸을 가리킨다. 세 몫을 시차를 두고 나눠 맡는 셈이다.

|arc|는 되돌아왔을 때 이어서 볼 호를 담는다. 되돌이 없이 깊이 우선 탐색을 하려면
꼭 있어야 하는 것이다.

@ 남은 것이 |low|인데, 여기가 재미있다. 알고리즘~T에서 이 밭은 두 얼굴을 갖는다.
$v$의 성분이 아직 정해지지 않았으면 $\.{LOW}(v)$를 담고, 정해지고 나면 그 성분의
우두머리 $v'$를 담는다---책에는 $\.{SENT}+v'$로 적혀 있다. 곧 우두머리를 담을
때는 모든 $\.{LOW}$ 값보다 큰 값이 되도록 밑값을 얹는다는 말이다.
...
@ 크누스의 \CEE/ 원본은 이 두 얼굴을 공용체로 구현했다. |low|는 |v.I|(정수)이고
...
@ 파수꾼도 마찬가지다. \.{SGB}의 |NewGraph|는 꼭짓점 배열을 언제나 넉넉하게
...
들고 있는 것이다. |vert[v].low|에 적혀 있을 수도 있고 아닐 수도 있다. 이 이중장부가
메모리 참조를 여럿 아낀다.
...
@ 뿌리를 찾는 훑기는 번호가 큰 쪽에서 작은 쪽으로 내려간다. |w|는 \.{T1}이
...
@ 새로 밟은 꼭짓점에 번호를 매기고, 그 $\.{LOW}$를 자기 번호로 둔다. |link|를
...
왜 그런가? $p=n$이면 모든 꼭짓점이 이 나무 안에 들어와 번호를 받았다는 뜻이다.
...
뿌리로, |lowv|를 $1$로 맞추어 \.{T8}로 뛰는 것이다. $\hbox{|lowv|}=1$이면
웅덩이에 있는 모든 것이 걷힌다.
...
@* 부모로 되돌아가기.
$v$의 화살을 다 보았으면 부모에게 돌아간다. 돌아가기 전에 두 가지를 한다:
$\.{LOW}(v)$가 $p(v)$와 같은지 보아 성분이 완성됐는지 가리고, 아니면 안쪽 화살을
내놓고 $\.{LOW}$를 부모에게 물려준다.

|vert[v].link|가 |sent|라는 것은 아무도 $v$의 $\.{LOW}$를 낮추지 못했다는 뜻이니,
...
경우가 셋이다. $\hbox{|lowv|}>\.{LOW}(u)$면 할 일이 없다.
$\hbox{|lowv|}<\.{LOW}(u)$면 $\.{LOW}(u)$를 낮추고, 겸해서 |vert[u].link|를
|null|로 지운다---군더더기 안쪽 화살을 찍지 않기 위해서다. $\.{LOW}(u)$를 $v$에게서
물려받았으니 $u$가 스스로 내놓을 화살은 없다.
...
두어도 된다고 생각했다. $v$가 이미 넉넉한 안쪽 화살을 내놓았으니 $u$가
\.{u->link}로 가는 화살을 또 내놓을 까닭이 없다고 여긴 것이다. 그것은 틀렸다.
$v$가 $u$의 낮은 값을 그대로 베껴 왔을 수 있고, 그렇다면 $v$는 $u$로 가는
...
\figcap{그림 3. 왼쪽이 옳은 답이다. $b\dadj c$를 빠뜨리면 남는 화살이
$c\dadj b\dadj a\dadj b$뿐이라, $a$에서도 $b$에서도 $c$로 돌아갈 길이 없다.}

@ $a$의 탐색이 끝났을 때를 보자. $a$는 화살 $a\dadj b$로 $\.{LOW}(a)=\.{LOW}(b)$를
...
돌아갈 수 없다. $a$가 내놓은 화살은 $b$까지밖에 데려다주지 않고, 거기서 앞으로
...
@* 성분 하나를 찍어낸다.
\.{T8}에 이르렀다는 것은 $v$가 우두머리이고 성분 하나가 완성됐다는 뜻이다. 웅덩이
...
|mems|를 둘 세는 것에 눈길이 갈 것이다. 하나는 방금 읽은 |vert[sink].low|의 몫이고
...
|settled| 목록은 웅덩이에서 걷어 낸 것들이 먼저 오고, 그 뒤에 $v$가, 그 뒤에
전에 있던 것들이 온다. 마지막에 |vert[t].link = v|로 두 토막을 잇는 까닭이다.
...
@ 밭을 다 펼쳐 보이는 것. |low|의 두 얼굴을 |k <= n|으로 가리는 대목을 눈여겨보라.
...
@ 스택 하나를 훑어 찍는 것. |printStack(sink)|이나 |printStack(settled)|처럼
부르면 된다. 고리가 잘못 이어져 있으면 그것도 알려 준다.
...
@ 잊지 말아야 할 것이 하나 더 있다. \.{.gb} 파일은 \CEE/ 쪽 \.{SGB}가 |save_graph|로
