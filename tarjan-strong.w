...
\GO/에는 공용체가 없다. 그렇다고 밭 둘로 쪼개면 ``지금 어느 얼굴인가''를 따로 물어야
...
$-666$). \GO/에서는 종료 코드를 작은 양수로 두는 것이 예의라 $1$에서 $4$까지로
...
달고 |goto|로 오간다. \GO/에도 |goto|가 있으니 우리도 그대로 둔다. 다른 데 같으면
...
이 한 줄은 \CEE/의 쉼표 연산자 |v->link=sink,sink=v,v=v->par|와 같은 일을 한다. \GO/는
...
아무 데서도 부르지 않는다. 필요할 때 손으로 한 줄 끼워 넣으라는 것이다. \GO/는
