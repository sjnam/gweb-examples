...
\def\title{동적 유향 해밀턴 회로}

@* 들어가며.
마디가 $\{1,2,\ldots,n\}$인 유향 그래프 $G$가 주어졌다고 하자. 이 프로그램은 동적
계획법을 조금 바꾼 방법으로, 유도 부분그래프 $G\mid\{1,2,\ldots,m\}$의 해밀턴 회로를
$m=1$, $2$, \dots,~$n$마다 모두 센다. 특히 $m=n$이면 $G$의 해밀턴 회로를 {\it 모두\/}
세는 셈이다. 무향 그래프를 다루는 \.{dynaham.w}의 유향판이다.

한번 알고 나면 착상은 단순한 편이지만 설명하기는 그리 쉽지 않다. 그래프 $G$의 호를
모은 집합이 다음 세 조건을 채우면 ``$m$-짜임''이라 부르자. (i)~$m$ 이하인 마디
$v$는 저마다 꼭 두 호에 나오는데, 하나는 $v$로 들어오고 하나는 $v$에서 나간다.
(ii)~두 끝이 다 $m$보다 큰 호는 없다. (iii)~방향이 있는 고리를 이루지 않는다. 그러면
$m$-짜임의 호들은 서로 떨어진, 방향이 있는 부분경로 몇 개를 이룬다.

그래프 $G$의 ``$m$-경계'' $F_m$은 $m$보다 크면서 $\{1,\ldots,m\}$에서 호를 어느
방향으로든 따라 닿을 수 있는 마디들의 집합이다. 어떤 $m$-짜임 안에서 $F_m$의 마디는
짜임 안의 차수가 $1$이냐 $2$냐 $0$이냐에 따라 ``바깥'', ``안쪽'', ``맨몸''으로
나뉜다. (부분경로가 $t$개이면 바깥 칸은 꼭 $2t$개다.) 바깥 마디는 ``샘''(source)이거나
``싱크''(sink)이고, 부분경로는 모두 싱크에서 샘으로 달린다. (크누스는 이 이름이
짐작과 거꾸로일지 모른다고 적었다. 하지만 부분경로를 늘릴 때는 싱크로 들어오는 새
호를 보태거나 샘에서 나가는 새 호를 보태고 싶기 때문에 이렇게 불렀다.)
...
동치류는 경계의 마디마다 정수 부호를 하나씩 붙인 수열로 나타낸다. 안쪽 칸의 부호는
$0$, 맨몸 칸은 $1$이고, $j$번째 부분경로의 바깥 칸은 $2j+[\hbox{샘}]$이다($1\le j\le
t$). 부분경로의 번호는 경계의 마디를 정해진 차례로 늘어놓았을 때 그 끝점이 처음 나오는
차례를 따라 매긴다. 부호의 뜻이 \.{dynaham.w}와 다르다는 점을 눈여겨보라. 거기서는
안쪽이 $-1$, 맨몸이 $0$, 바깥이 $j$였다.

모든 $(m-1)$-짜임의 셈을 이미 알고 있다면 모든 $m$-짜임의 동치류를 찾아 세는 것은
어렵지 않다. $(m-1)$-짜임의 ``뒤''는 마디 $m$이 바깥이냐 안쪽이냐 맨몸이냐에 따라 새
호를 $1$개, $0$개, $2$개 보탤 뿐이기 때문이다.

@ 크누스가 붙인 내력을 옮긴다.

\medskip{\narrower\noindent
[{\it 내력:}\enspace 이 프로그램은 Peter Weigel과 Don Knuth가 2025년에 지은
{\mc DYNABIDIHAM}을 고친 것이다. 유향 그래프의 회로는 양방향 그래프 회로의 특수한
경우이니 바탕 알고리즘은 거의 같다. 바탕 알고리즘을 단순하게 다듬으려 하지는 않았다.
하지만 메모리는 절반 가까이 아낄 수 있다. 양방향 그래프에서는 보통의 한 방향짜리
말고도 샘과 샘 사이, 싱크와 싱크 사이의 부분경로가 있을 수 있는데, 유향 그래프에서는
그런 있을 수 없는 상태를 기억할 자리를 마련할 필요가 없기 때문이다.

{\mc DYNABIDIHAM}과 다른 점이 하나 더 있다. 이 프로그램의 입력은 표준 스탠퍼드
그래프베이스 형식의 어떤 그래프든 될 수 있다. 양방향 그래프는 그와 달리 호와 변의
길이 필드에 특별한 부호를 넣어 적어야 한다.]\par}\medskip

@ 이것은 크누스의 \.{CWEB} 프로그램 \pdfURL{\.{dynadiham.w}}%
{https://www-cs-faculty.stanford.edu/\TILDE/knuth/programs/dynadiham.w}를
\.{GWEB}으로 옮긴 것이다. 원본의 머리글 \.{Last-Modified}는
\.{Tue, 30 Dec 2025 01:41:25 GMT}다. \.{dynaham.w}보다 넉 달쯤 앞선다.

뼈대는 \.{dynaham.w}와 같다. 경계와 트라이와 압축 방식, 옛 동치류를 찾아가 뒤를
보태는 흐름이 모두 그대로다. 다른 곳은 세 군데다. 입력한 유향 그래프에 반대 방향의
짝 호를 보태 양방향 그래프로 바꾸는 자리, 부호와 트라이의 모양, 그리고 호의 방향을
맞춰 보며 유도된 변을 보태는 자리다. 이 판도 \.{dynaham.w}의 글과 코드를 가져다 쓰고
다른 곳만 고쳤다. 트라이의 긴 예는 \.{dynaham.w}에 옮겨 두었다.
...
\.{dynaham.w}에서 고친 결함 넷 가운데 둘은 이 원본에 없다. 제 고리는 처음부터
거절하고, 겹친 호는 |add_derived|가 바깥 마디에 호 셋이 붙는 경우를 따로 걸러
준다. 나머지 둘, 곧 압축한 트라이의 넘침을 제때 알아채지 못하는 것과 마디 없는
그래프에서 죽는 것은 그대로 있어서 똑같이 고쳤다. 큰 수가 넘칠 때 찍는 말의 오타
하나도 고쳤다.
...
@<호 목록을 양방향 꼴로 바꾼다@>@;
fmt.Printf("Dynamic Hamiltonian cycles of the directed graph %s", g.ID)
fmt.Printf(" (%d vertices, %d arcs):\n", n, g.M/2)

@ 호 $u\dadj v$는 $u$에 닿은 호 목록에 끝이 $v$인 항목으로 적힌다. 이 프로그램의
자료구조는 그 {\it 반대\/} 호 $v\dadj u$도 함께 나타낸다. $v$에 닿은 호 목록에 끝이
$u$인 항목을 하나 넣는 것이다.

``진짜'' 호와 ``반대'' 호는 \.{SGB}의 \&{Arc} 레코드의 길이 필드 |len|으로 가른다.
입력 그래프에서 길이가 $l$인 진짜 호는 길이가 $4l+1$인 호로 바꾸고, 그 반대 호는
길이 $4l+3$으로 적는다. (본래 길이 $l$은 이 프로그램의 동작에 아무 영향도 없지만,
프로그램을 넓힐 때 쓸지 몰라 남겨 둔다.)

이렇게 호를 겹으로 적고 길이를 바꾸면 주어진 유향 그래프를 안에서 양방향 그래프로
바꾼 셈이다. 그래서 {\mc DYNABIDIHAM}의 알고리즘을 쓸 수 있다. 길이를 $4$로 나눈
나머지의 뜻은 \.{ssbidiham.w}에서 정한 것과 같다. 진짜 호 $u\dadj v$는 $u$의
목록에서 길이가 $1$이니 $u\0<<v$이고, 반대 호는 $v$의 목록에서 길이가 $3$이니
$v\0>>u$, 곧 같은 변이다.

첫 바퀴에서 모든 진짜 호의 길이를 바꾸고, 둘째 바퀴에서 반대 호를 보탠다. 반대 호는
$v$의 목록 앞머리에 붙으니, 둘째 바퀴가 나중에 $v$의 목록을 훑을 때 그것들을 먼저
만난다. 길이가 $3$(을 $4$로 나눈 나머지)인 것은 건너뛴다. 원본의 두 바퀴는 제 고리를
만나면 그만둔다. 음수 길이도 $4l+1$을 $4$로 나눈 나머지가 $2$의 보수로 $1$이니 탈이
없다.

@<호 목록을 양방향 꼴로...@>=
for u := 0; u < n; u++ {
	for a := g.Vertices[u].Arcs; a != nil; a = a.Next {
		if v := int(g.Index(a.Tip)); v == u {
			fmt.Fprintf(errw, "graph has a self-loop %s--%s (length %d)!\n",
				g.Vertices[u].Name, g.Vertices[v].Name, a.Len)
			quit(-4)
		}
		a.Len = 4*a.Len + 1
	}
...
for u := 0; u < n; u++ {
	for a := g.Vertices[u].Arcs; a != nil; a = a.Next {
		if a.Len&0x3 == 1 {
			g.NewArc(a.Tip, &g.Vertices[u], a.Len+2)
		}
	}
}
...
하나일 수밖에 없다. 그리고 앞머리 2053 다음에는 $0$, $1$, $4$, $6$, $7$만 올 수 있다.
($2$나 $3$은 올 수 없다. 1번 부분경로의 부호는 $\{2,3\}$인데 둘 다 이미 나왔기
때문이다. $5$도 올 수 없다. 2번 부분경로의 부호는 $\{4,5\}$인데 $5$가 이미 나왔기
때문이다.)
...
한 번 나왔고 두 번은 아직이면 |c=tmap[itmap[c]]|다. 논리를 되짚으려고 나오지 않은
쪽의 부호도 |tmiss[c]|에 적어 둔다.

수준 $l$에서 앞머리 $a_0\ldots a_{l-1}$의 마디는 |mem|의 색인인 포인터
$p=p(a_0\ldots a_{l-1})$로 가려낸다. 부호 $a_l$이 될 수 있는 값이 차례로 $0$, $1$,
|tmiss[tmap[0]]|, \dots, |tmiss[tmap[tms-1]]|, $2|tmx|$, $2|tmx|+1$이면, 자식 마디
$p'=p(a_0\ldots a_l)$은 차례로 |mem[p]|, |mem[p+1]|, \dots, |mem[p+tms+3]|에 든다. 그
...
var (
	...
	tmiss, omiss             [deg]int // $2c$와 $2c+1$ 가운데 아직 나오지 않은 쪽
)
...
tmap[tms], itmap[tmx], tmiss[tmx] = tmx, tms, j^1
tms++
tmx++
pp = p + 1 + uint32(tms) + uint32(j&1)

@ 끝점의 두 번째 부호는 첫 번째와 샘·싱크가 반대여야 한다. 아니면 무언가 잘못한
것이다.

@<|j/2|를 |tmap|에서...@>=
if j != tmiss[j/2] {
	fmt.Fprintf(errw, "Parity mismatch!\n") // 무언가 잘못했다
	quit(-666666)
}
tms--
...
@ 미묘한 점이 하나 있다. |l+2+tms=q|이면 $2|tmx|$와 $2|tmx|+1$을 위한 칸을 두지
않는다. 끝점 |tmx|는 두 번 나와야 하는데 남은 자리가 하나뿐이기 때문이다.
...
if j >= 2 {
	if j >= 2+oms {
		...
		oms--
		kk, kkk = omap[oms], omap[j-2]
		omap[j-2], iomap[kk] = kk, j-2
	}
}
compress(l+1, mem[int(p)+j], childDeg(l, oms, q))
if j >= 2 { // 성긴 집합에 한 일을 되돌려야 한다
	if kk == 0 {
		oms--
		omx--
	} else {
		omap[j-2], omap[oms], iomap[kk] = kkk, kk, oms
		oms++
	}
}

@ 수준 |l+1|의 자식이 몇 갈래인지는 압축과 풀기가 똑같이 셈한다. 남은 자리가 열린
끝점의 수와 같으면 열린 끝점만 올 수 있고, 하나 더 많으면 $0$과 $1$이 더해지며,
그보다 많으면 새 끝점의 두 부호도 올 수 있다.
...
|oldmemsize|만 $800$으로 줄이고 \.{AddressSanitizer}를 붙여 마디 $30$개짜리 무작위
유향 그래프를 돌려 보니, 넘침을 알리지 못하고 힙 넘침으로 잡혔다. 이 판은 낱말 수
|oldmemsize/8|과 견준다.
...
@ 새 끝점의 자식은 둘이다. 자리 |2+oms|가 $2|omx|+0$(싱크)을, |3+oms|가
$2|omx|+1$(샘)을 나타낸다. 이 끝점이 다시 나올 때는 반대 부호여야 하니 그것을
|omiss|에 적어 둔다.
...
case j < 2:
	oldcode[l] = j
case j >= 2+oms:
	omiss[omx] = 2*omx + 3 + oms - j
	oldcode[l] = omiss[omx] ^ 1
	omap[oms], iomap[omx] = omx, oms
	omx++
	oms++
	kk = 0
...
정했다. 여기서 $u\adj v$는 주어진 유향 그래프에 $u$에서 $v$로 가는 호나 그 거꾸로
가는 호가 있다는 뜻이다. 그러니 $F_0=\emptyset$이고
$F_m=\bigl(F_{m-1}\cup\{v\mid m\adj v$이고 $m<v\}\bigr)\setminus m$이다. 특히
$F_1=\{v\mid 1\adj v\}$은 마디 $1$의 방향을 따지지 않은 이웃 집합이다. 눈여겨보면
...
@ 마디 $m$의 호 목록에는 이제 진짜 호와 반대 호가 다 들어 있으니, 호의 방향과
상관없이 이웃을 모두 만난다. 제 고리는 그래프를 읽을 때 이미 거절했으니 $k=m$인
이웃은 없다.
...
(실은 방향도 맞춰야 한다. 새 호는 싱크로 들어오거나 샘에서 나가야 한다. 크누스는 이
코드가 {\mc DYNABIDIHAM}에서 옮겨 온 것이라 필요한 것보다 일반적이라고 적었다. 실제로는
{\it 양방향\/}으로 맞는지를 따진다.)
...
@* 해 보기.
서로 다른 두 마디 사이에 두 방향의 호가 다 있는 완전 유향 그래프에는 해밀턴 회로가
$(n-1)!$개 있다. 마디가 일곱이면 $720$개인데, 프로그램도 $720$개를 찾는다.

무향 그래프를 주면 변 하나가 두 방향의 호로 적혀 있으니 두 방향의 호가 다 난 유향
그래프로 읽힌다. 그러면 무향 회로 하나가 도는 방향에 따라 둘이 된다. $6\times6$ 판의
나이트 그래프에서 찾은 $19724$개는 \.{dynaham.w}가 센 $9862$개의 두 배이고,
\.{ssdiham.w}가 되추적으로 찾은 수와 같다. $6\times8$ 판에서 찾은 $110976284$개도
$55488142$의 두 배다. 다만 \.{dynaham.w}보다 느리다. $6\times6$ 판은 이 판으로
$10.6$초, $6\times8$ 판은 원본이 $50$초, 이 판이 $69$초 걸린다. 샘과 싱크를 가르느라
동치류가 더 많기 때문이다.
...
\smallskip
\item{$\bullet$} 원본이 옳은지는 파이썬으로 짠 비트 집합 동적 계획법과 견주어 보았다.
처음 $m$개 마디로 된 유도 부분그래프마다 유향 해밀턴 회로를 곧이곧대로 센다. 겹친 호가
있는 것을 섞은 무작위 유향 그래프 $30$개와 작은 그래프 일곱에서 원본이 모두 맞았다.
\item{$\bullet$} 그 무작위 그래프들에 \.{AddressSanitizer}와
\.{UndefinedBehaviorSanitizer}를 붙여 돌려도 경고가 없었다. 마디 없는 그래프에서만
힙 넘침이 났다.
\item{$\bullet$} 이 판은 앞의 세 곳을 고친 \CEE/ 판과 그래프 $114$개, 잘못된 명령줄
다섯에서 모두 같았다. 그래프는 무작위 유향 그래프 $30$개, \.{ssdiham.w}를 시험한 그래프,
나이트 판들이다. 고치기 전의 원본과도 마디 없는 그래프만 빼고 모두 같았다.
\smallskip
