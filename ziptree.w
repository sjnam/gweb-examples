...
키는 모두 자기보다 작고 오른쪽 부분트리의 키는 모두 자기보다 크다(대칭 순서).
여기에 더해 노드마다 정수 {\it 랭크}(rank)를 하나씩 두고, 트리가 랭크에 대해
{\it 최대 힙} 꼴이 되도록 유지한다. 곧 어떤 노드의 랭크는 그 자식들의 랭크보다
크거나 같다. 키는 검색을, 랭크는 균형을 맡는 셈이다.

랭크를 {\it 무작위로} 고르되 그 분포를 잘 잡으면, 트리의 모양은 거의 늘
납작해진다. Zip 트리의 기대 깊이는 $1.5\lg n$ 남짓으로, $n$이 아무리 커도
검색과 갱신이 $O(\log n)$에 끝난다. 같은 성질을 가진 자료구조로 {\it 트립}
(treap)과 {\it 스킵 리스트}(skip list)가 있는데, Zip 트리는 트립과 본질적으로
같으면서 랭크를 {\it 기하분포}에서 뽑아 노드마다 필요한 비트 수를 $O(\log n)$
에서 $O(\log\log n)$으로 줄였고, 스킵 리스트와는 자연스러운 동형 관계를 이룬다.

이 글의 백미는 갱신 방법이다. 보통의 균형 트리가 {\it 회전}(rotation)으로
모양을 고치는 것과 달리, Zip 트리는 검색 경로를 키 기준으로 두 갈래로 {\it 가르고}
...
자료구조 묶음이므로 |main|이 없다. 대신 구현이 제대로 도는지 확인하는 검사
프로그램을 따로 둔다(맨 끝의 \.{ziptree\_test.go}). \.{ziptree.go}의 뼈대는 이렇다.
@c
package ziptree
...
분포일까? 노드가 $n=2^{h+1}-1$개인 {\it 완전} 이진 트리에서 높이가 |k|인 노드의
...
그래서 Zip 트리는 {\it 역사 독립적}이다. 어떤 순서로 넣고 뺐든 지금 든 노드들의
...
Theorem 1을 떠올리면 더 줄일 여지가 보인다. Zip 트리의 최대 랭크는 거의 확실히
...
@* 검사와 실험.
Zip 트리가 제대로 도는지 확인하고 이런저런 활용을 실험하려면, 굳이 표준 입력을
...
넣고(그림을 재현하려면 적힌 랭크를 그대로 박아야 한다), |shape|는 트리를 한 줄
