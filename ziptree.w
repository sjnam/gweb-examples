...
프로그램 \.{ziptree\_test.go}를 따로 둔다. 파일 \.{ziptree.go}의 뼈대는 이렇다.
@c
package ziptree
...
랭크가 큰 쪽을 위에 놓고, 그 자리를 채운 뒤 나머지를 재귀로 마저 엮는다. 쪽 |x|가
...
앞의 Theorem 1을 떠올리면 더 줄일 여지가 보인다. 짚트리의 최대 랭크는 거의 확실히
...
실제로 줄었음을 보인다. 줄이는 대신 잃는 것도 있다. 우선 |t.nodes[root].left| 같은
...
받는 |main|을 둘 필요가 없다. 여기서 \.{GWEB}의 \.{@(ziptree\_test.go@>} 표기를 쓰면 이 묶음을 별도
...
@ 검사에 쓸 도우미들이다. 함수 |insertRanked|는 랭크를 무작위로 뽑는 대신 밖에서 받아
...
모양을 모두 그림과 글자 그대로 견준다. 함수 |t.Log|로 그림도 함께 찍으므로
...
@ 키 \.{K}는 코드값으로 \.{J}와 \.{L} 사이에 든다. 랭크 3은 만나는 \.{H}, \.{M}(랭크 2)보다
...
@ 인덱스 구현을 검사하려면 도우미 둘이 더 필요하다. 함수 |insertRankedA|는 아레나에
...
@ 함수 |profile|은 트리를 한 번 세우는 동안 |runtime.ReadMemStats|로 ① 객체 할당 횟수,
② GC 횟수, ③ 다 짓고 GC한 뒤 유지하는 힙을 잰다. 그리고 |KeepAlive|로 측정이 끝나기 전에
트리가 수거되지 않게 막는다.
@<할당 벤치마크@>=
...
@ 명령 \.{go test -bench=.} 로 돌리면 한 삽입에 드는 시간과 할당이 나란히
찍힌다. 아레나판의 |allocs/op|이 0에 수렴하는 것이 눈에 띈다.
@<할당 벤치마크@>=
