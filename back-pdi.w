...
수 $153$은 제 자릿수를 저마다 세제곱해 더하면 도로 제자신이 되는 놀라운 수이다.
곧 $153=1^3+5^3+3^3$이다. 이런 수를 {\it 완전 자릿수 불변수\/}(perfect digital
...
부등식 $10^p\le x<10^{p+1}$이면 $x$는 $p+1$자리인데, $p>m$일 때는
...
처럼 {\it 내림차순\/}으로 하나씩 고르며 백트래킹한다. 연산 $\pi_m$은 자릿수의 순서에
무관하니, 순서를 정해 두면 같은 다중집합을 한 번씩만 훑게 된다. 차수 $m$이 작다면
...
@ 전역 변수는 대부분 레벨을 첨자로 갖는 큰 배열이다. 문장 |goto|가 선언을
건너뛰지 못하는 Go의 제약 때문에, 탐색이 쓰는 작업 배열은 패키지 전역으로 둔다.
레벨 |l|은
...
왜 못 만들겠는가? 함수 |kmult|는 큰 수 |a|를 한 자릿수 |k|배로 곱한다---모두 덧셈으로.
@<보조 루틴@>=
...
@ 이제 표를 채운다. 칸 |table[1][k]|를 $k$에서 시작해 |k|배씩 $m-1$번 곱하면 $k^m$이
...
함수 |add|가 다루는 수는 |mdigs| 워드, 곧 십진 $15\cdot|mdigs|$자리까지다. 우리가
...
끝으로 사소하지만 요긴한 성질 하나. 함수 |add|는 |p|, |q|, |r|가 모두 같은 배열이어도
옳게 돈다. 각 워드에서 읽기를 쓰기보다 먼저 마치기 때문이다. 그래서 |kmult|가
...
다 떨어졌으면 |b5|가 한 레벨 물러나며 상태를 되돌린다. 레벨 |l|이 $0$까지 줄면 탐색이
끝난다.
@<모든 경우를 백트래킹한다@>=
b4:
	if xl != 0 {
...
핵심 발상은 경계 $a_l$과 $b_l$을 계산해 두는 데 있다. 앞의 $x_1,\ldots,x_{l-1}$이 주어진
...
자릿수 |d|는 $D$에 |dist[l][d]|번, $P$에 |pdist[l][d]|번 나타난다. 자릿수가 |d>xl|이면
|pdist[l][d]<=dist[l][d]|여야 한다. 자릿수가 |d=xl|이면
$|pd|=\max(0,|pdist[l][d]|-|dist[l][d]|)$로 둔다. 이를테면 |xl|이 $D$에는 세 번, $P$에는 한 번 나타나면 $pd=0$이지만, $P$에 세
번, $D$에 한 번이면 $pd=2$가 되어 $x_l=x_{l+1}=|xl|$을 {\it 강제로\/} 골라야 한다.

@ |r|을 |x|의 아직 모르는 자릿수 개수라 하자. (|pd=0|일 때 이는 $m+1$에서
$\vert S\vert$를 뺀 값이다.) 경계가 $a_{lt}<b_{lt}<|xl|$이면 $r>0$이고, 모르는 자릿수 하나가
$a_{lt}$와 $b_{lt}$ 사이(양끝 포함)에 놓인다.

문턱값 |xl|이 줄면 경계가 조여지고, 그러면 접두가 더 길어질 수 있다. 그것이 바로 우리가
...
@ 여기가 가장 섬세하고 가장 중요한 대목이다. 여기서 |x|의 자릿수 하나를 새로 알게 되어
접두를 한 자리 늘린다.

덧붙여, |goto| 없이는 코드를 되풀이하지 않고 짜기 어려운 ``흐름도''의 흥미로운
예이기도 하다. 두 조건 $A$, $B$와 두 동작 $\alpha$, $\beta$가 있다고 하자. 조건이 $A$이고
$B$이면 $\alpha$ 다음 $\beta$를, $A$이고 $B$가 아니면 아무것도, $A$가 아니면 $\beta$만
하고 싶다. 그런데 |goto| 없이 하려면 $A$를 두 번 평가하거나 $\beta$를 두 번 적어야 한다.
@<현재 접두를 늘리거나 |b5|로 간다@>=
mems++; p = pdist[l][blt]
if blt >= xl {
	mems++
