...
크누스는 이렇게 적는다. $w$가 $v$에서 닿을 수 있으면 `$v\Rightarrow w$', 아니면
...
|src|는 조금 더 재미있다. 그 밭이 나중에 지게 될 몫은 ``내가 든 조각에서 내 다음
...
모르니, 일단 바로 앞에 나온 성분 |prev|를 적어 둔다. $v$가 아래쪽에서 조각에
...
책의 단계는 \.{W1}부터 \.{W8}까지 여덟이지만 여기 이름표는 여섯뿐이다. \.{W1}은
앞 장에서 본 ``성분 하나를 찍어낸다''가 이미 하고 있고, \.{W8}은 사라졌기
때문이다. \.{W8}이 하는 일은 둘인데, $\.{WP}\gets v$는 연습문제~92가 없어도
된다고 일러 준 것이라 빼고---그래서 이 프로그램의 |wp|는 새로 연 조각이 아니라
그 {\it 위\/} 조각의 우두머리다---, $u\gets u''$는 우리가 안쪽 |u|를 따로
선언해 바깥 것을 건드리지 않았으니 할 일이 없다. \.{HIT}을 경우~$1'$에서만
...
그래서 |src| 길을 걸으며 근원마다 |whit|을 본다. |whit|이 서 있지 않은 근원을
