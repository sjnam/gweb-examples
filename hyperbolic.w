...
그릴 {\logo METAPOST} 파일을 그 이름으로 적어 준다.
...
것인데, 쓸모가 둘이다. 칸 |rest[i]|는 변~$i$ 말고 남은 두 변이고, |rest[j]|는 또한
...
@ 첫 그림은 우리가 셈해 낸 쌍곡 직선을 그대로 그린 것이다. 직선 $1$번만 빼는데,
...
@ 이번에는 하나도 빼지 않는다. 직선 $1$번의 되비침은 $\vert z\vert=1/r^2$이라는
어엿한 원이라 그릴 값이 있다.
