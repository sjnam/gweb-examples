...
@* 들어가며.
\pdfURL{코드포스 1093E}{https://codeforces.com/problemset/problem/1093/E} 문제다.
길이 |n|의 두 순열 |a|와 |b|가 있고, |q|개의 질의를 처리한다.
\smallskip
{\narrower
...
$posA(v)$번째, $b$에서는 $posB(v)$번째에 있다고 하면, $v$에 점
$(posA(v),\,posB(v))$를 대응시킨다. 그러면 ``$v$가
$a[l_a\,..\,r_a]$에 있다''는 것은 $posA(v)\in[l_a,r_a]$와 같고,
``$b[l_b\,..\,r_b]$에 있다''는 것은 $posB(v)\in[l_b,r_b]$와 같다. 따라서
질의 1은 직사각형 $[l_a,r_a]\times[l_b,r_b]$ 안의 점 개수를 세는 일이 된다.

질의 2는 어떤가. $b$에서 두 자리를 맞바꾸면 거기 있던 두 값의 $posB$만
바뀌므로, $posA$는 그대로인 채 두 점의 $y$좌표만 옮겨진다. 곧 {\it 점 두
