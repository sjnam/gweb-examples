...
* [li-ruskey.w](li-ruskey.w) — 앞의 `koda-ruskey`의
  ...
  넷.
* [lp.w](lp.w) — 크누스가 심플렉스 알고리즘을 몸으로 익히려고 짠 선형 계획
  프로그램(2005)이다. 여유 변수를 넣은 표에서 0행이 모두 음 아닌 수가 될 때까지
  피벗하고, 피벗 행은 사전순으로 가장 작은 r_i/a_ij로 골라 되풀이를 피한다.
  정수 표(열마다 배율을 두어 분수를 피함)와 부동소수점 표를 나란히 돌려 서로
  확인하고, 쌍대 문제의 해도 함께 읽는다. 원본의 결함을 고쳤다. 음수 상수항
  검사가 엉뚱한 칸을 읽어 해 없는 문제에 엉터리 답을 내는 것, 숫자가 아닌 글자에서
  끝없이 돌며 배열 밖에 쓰는 것, 줄바꿈 없는 마지막 줄을 거절하는 것, long을
  %d로 찍어 2³¹ 이상이 잘리는 것, 정수 방법의 64비트 넘침을 모르는 것이다.
  설명에 있던 표와 식의 잘못 여럿도 바로잡았다. 무작위 문제 1000개에서 최적해
  418개를 쌍대성 증명서로 확인했고, 넘치지 않은 839개는 원본과 바이트까지 같다.
