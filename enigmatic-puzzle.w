...
그렇지만 이 계산은 아름답도록 잘 쪼개진다. 바깥 고리가 훑는 회전자 배치와 회전량은
서로 아무것도 주고받지 않는다. 그래서 그것들을 {\it 칸\/}으로 썰어 고루틴 여럿에게
나누어 맡겼다. 코어가 열 개인 내 기계에서 일곱 배 반을 넘게 빨라진다.

까다로운 것은 셈이 아니라 출력이다. 원본은 훑으면서 찍는데, 무엇을 찍을지가 그때까지
본 것에 달려 있어 순서를 탄다. 그 실타래를 푸는 이야기가 「되풀기」 장이다. 결론만
미리 적자면, 찍히는 것은 한 줄기로 돌 때와 한 바이트도 다르지 않고 찍히는 때도
거의 그대로다.
...
@ 칸 하나는 회전자 배치 하나와 느린 회전자의 회전량 $p_0$ 하나를 맡는다. 배치가
$60$가지에 $p_0$가 $26$가지이니 칸이 $1560$개다. 회전자 배치만으로 갈라 $60$칸으로
두어도 나누어 맡기는 데에는 넉넉한데, 굳이 잘게 썬 까닭은 뒤의 「되풀기」 장에서 밝힌다.

워커는 칸을 하나 집어 훑고, 훑은 결과를 그 칸에 적어 둔다. 적어 두는 것은 세 가지다.
이 칸에서 들여다본 평문이 몇 개인가, 그 평문마다 회전량이 무엇이었는가, 그리고 찍힐
만한 후보가 무엇이었는가.

@<자료형@>=
type unit struct {
	r0, r1, r2 int      // 이 칸이 맡은 회전자 배치
	p0         int      // 이 칸이 맡은 느린 회전자의 회전량
	done       bool     // 다 훑었는가
...
@ 칸 목록은 원본의 바깥 네 겹 고리를 그대로 펴 놓은 것이다. 목록의 차례가 곧 원본이
훑던 차례이고, 이것이 뒤에서 출력을 되살리는 밑바탕이 된다.
...
for r0 := 0; r0 < 5; r0++ {
	for r1 := 0; r1 < 5; r1++ {
		...
		for r2 := 0; r2 < 5; r2++ {
			...
			for p0 := 0; p0 < 26; p0++ {
				units = append(units, unit{r0: r0, r1: r1, r2: r2, p0: p0})
			}
		}
	}
}
...
칸을 하나 다 훑으면 그 자리에서 앞자락을 찍어 본다. 찍는 일은 |emit|을 쥔 하나만
할 수 있고, 그동안 |ndone|과 |best|와 |base|를 함께 건사한다. 차례로 이미 찍은 칸의
수가 |ndone|, 여태까지의 최고점이 |best|, 여태까지 들여다본 평문의 수가 |base|다.
...
곧 백만 남짓이다. 원본에서 여섯 겹이던 이 고리가 여기서는 둘로 줄었다. 바깥 네 겹은
칸 목록이 되어 워커들에게 흩어졌기 때문이다.
...
func (w *worker) scan(un *unit) {
	...
	var k, jj, kk, p1, p2, pp int
	r0, r1, r2, p0 := un.r0, un.r1, un.r2, un.p0
...
@ 느린 회전자의 회전량 $p_0$은 칸이 정해 주었으니, 남은 두 개만 훑는다.
...
@* 되풀기.
찍을 것을 고르는 일은 앞을 본다. 후보 하나가 찍히느냐는 그때까지의 최고점에 달렸고,
줄 끝의 일련번호는 그때까지 들여다본 평문의 수다. 그래서 칸 $k$의 출력은 칸 $0$부터
$k-1$까지가 모두 끝나야 확정된다.

뒤집어 말하면, {\it 끝난 앞자락까지는 바로 찍어도 된다\/}. 칸을 하나 끝낸 워커는
그것을 끝났다고 적어 놓고, 아직 안 찍은 가장 앞선 칸부터 끝난 데까지 죽 찍어 낸다.
찍는 차례가 칸 번호 순서이므로 결과는 한 줄기로 돌 때와 똑같다.

이것이 칸을 $60$개가 아니라 $1560$개로 썬 까닭이다. 늦어지는 정도는 아직 안 끝난 가장
앞선 칸 하나가 정하는데, 칸이 $60$개면 그 하나가 전체의 $1/60$이라 출력이 여섯 번쯤에
몰려 나온다. $1560$개면 첫 줄이 $6$초 만에 나오고 그 뒤로는 찾는 대로 흘러나온다.

칸마다 |enc| 표를 다시 지어야 하니 값이 들 법도 한데, 한 벌에 $4$밀리초라 통틀어
$6$초, 전체의 $0.6$퍼센트다. 오히려 재어 보면 잘게 썬 쪽이 빨랐다---$60$칸일 때
$16$분 $39$초, $1560$칸일 때 $16$분 $10$초다. 칸이 잘아야 맨 끝에 남는 꼬리가
짧아지기 때문이다.
...
다 찍은 칸은 갈무리해 둔 것을 놓아준다. 안 찍은 칸이 일꾼 수의 두 배를 넘지 않으므로,
칸을 아무리 잘게 썰어도 쥐고 있는 양이 늘지 않는다.
...
|runtime.NumCPU|이 열을 내놓는다. 거기서 벽시계로 $16$분 $10$초가 걸렸다. 나누어
맡기기 전에는 같은 기계에서 $2$시간 $4$분 $26$초가 걸렸으니 $7.7$배다. 첫 후보는
$6.4$초 만에 나오고, 그 뒤로는 찾는 대로 흘러나온다.
...
@ 튜링과 Welchman의 봄베가 1940년에 하던 일을, 지금은 노트북 한 대가 십육 분이면
...
그러고 보면 이 글이 마지막에 한 일도 그들이 하던 것과 다르지 않다. 한 대로는 벅차니
여러 대에 나누어 맡긴 것이다. 그들은 봄베를 늘렸고 우리는 코어를 열 개 썼을 뿐이다.
