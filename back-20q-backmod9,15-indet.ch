...
%
%   gtangle back-20q.w back-20q-backmod9,15-indet.ch
%   gweave  back-20q.w back-20q-backmod9,15-indet.ch
%
% 위의 두 가지에 더해, 20번의 (D)("정해지지 않는다")를 거짓이 아니라
% 참이라고 놓는다.
...
@y
문제는 따로 손을 봐 주어야 한다. 여기서는 재미 삼아 \opt{D}가 언제나
{\it 참\/}이라고 놓아 본다. 그리고 \opt{E}는 거짓이라고 놓는데, 그 가정은 이
프로그램의 출력을 살펴 확인해야 한다. \opt{A}, \opt{B}, \opt{C}에 대해서는
문제를 살짝
고쳐 읽는다---``최고 점수''가 아니라 ``이번 판이 내놓는 모든 답안지의 점수''로.
@z
@x
...
@y
@* 돌려 보기.
이것은 9번과 15번을 손보고, 거기에 20번의 \opt{D}를 참으로 놓은 변종이다.
먼저 스무 문제를 모두 맞히는 답안지를 찾아보자.
$$\.{\$ back-20q 0 0}$$
여기서도 답이 하나도 없다. 열아홉 개까지 내려가면 두 장이 나온다. 19번을
틀렸을 때 한 장, 20번을 틀렸을 때 한 장. 그러니 최고 점수는 여전히
{\it 19점\/}이다.
...
@ 명령줄이 고를 수 있는 211가지 무늬를 모두 돌리면 답안지가 모두 139장 나온다.
그 가운데 19점짜리가 2장, 나머지 137장은 18점짜리다. 20번의 \opt{D}를 참으로
열어 준 것만으로 63장이 139장으로 늘었다.
@z
@x
...
다르지 않았다. mem 수는 다섯 경우에서 $1$이나 $2$만큼 달랐는데, 19번의 미룬
확인에서 원문이 논리곱 대신 비트곱을 써 짧은 회로가 끊기지 않는 자리 때문이다.
@z
@x
...
@y
둘째, 이 프로그램과 아무것도 나눠 쓰지 않는 채점기를 따로 만들어, 나온 139장을
한 장씩 스무 문제의 우리말 뜻에서 곧바로 다시 채점했다. 대문자로 적힌 답은
주장이 참, 소문자로 적힌 답은 거짓이어야 한다. 139장 모두 맞았다.
@z
