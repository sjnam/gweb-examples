...
통째로 한 덩어리로 뒤집는다\/}를 할 수 있을 때까지 되풀이한다. 카드 $1$이 맨
위에 나타나면 게임 끝이다.
...
이 수는 성큼성큼 자란다. 첫 항 $f(1),\ldots,f(15)$는
$$0,\;1,\;2,\;4,\;7,\;10,\;16,\;22,\;30,\;38,\;51,\;65,\;80,\;101,\;113$$
이고({\sl 온라인 정수열 사전\/}(OEIS)의 수열 \.{A000375}), 그다음 $f(16)=139$부터는
컴퓨터를 한참 부려야 겨우 나온다. 작은 $n$에서만 알려져 있는, 심심풀이
게임치고는 어엿한 미해결 문제인 셈이다.
...
값 $N$은 $2^{n+1}$ 아래에 갇힌 채 수마다 커지니 게임은 유한하다. 덤으로 점수가
$2^{n+1}$을 넘지 못한다는---후하디후한---상계까지 공짜로 얻는다.

@* 거꾸로 풀기.
값 $f(15)$가 알고 싶다고 $15!\approx1.3$조 가지 시작 패를 모두 두어 볼 수는 없는
...
그림 1을 {\it 오른쪽에서 왼쪽으로\/} 읽으면 이 모두가 눈에 보인다. 패 $2\,1\,3$의
...
기록한다. 자리 $0$을 영원히 지키는 $1$은 굳이 적지 않는다. 이제 $k+1$장 뒤집기를
...
자유석 표시니 남은 아무 카드나 채워 읽으면 된다. 덱 크기 |n|을 $3$으로 줄여 돌리면
\smallskip
\centerline{\.{1: 2 1 0}\qquad\.{2: 3 1 2}\qquad\.{2: 2 3 1}}
\smallskip
\noindent 이 차례로 나온다---그림 2의 나무를 왼쪽 가지부터 걸은 순서
그대로다. 덱 크기를 $n=15$로 두면 한참을 달리며 기록을 차츰 갈아치우다 끝내
$f(15)=113$에 다다른다.
@<신기록 순열을 출력한다@>=
fmt.Printf("%d:", l+1)
for j := range n {
