...
\thing{\<타일>\.*\<타일>}
  ...
  이를테면 $z=1+2i$일 때 \.{3,2*-2,3}은 \.{-8,1}을 준다. 셈이
  $(3+2i)*(-2+3i)=i(3+2i)+z(-2+2i)=-8+i$이기 때문이다.
...
\thing{\.v\<정수>}
  말수를 정한다. 명령 \.{v0}이 가장 조용하고 \.{v-1}이 가장 수다스럽다.
...
@ 말수는 비트 세 개로 정한다. 명령 \.{v-1}이면 모든 비트가 서므로 가장
수다스럽다.
...
같은 것을 {\it 접기열\/}로도 적을 수 있다. 여기서 $f_k$는 $d_k$에서 $d_{k+1}$로 트는
...
@ 가우스 정수를 사람이 읽는 꼴로 적는다. 계수가 $1$과 $-1$이면 적지 않고, 실수부가
...
@ 곱셈 자체. 변수 |j|가 사본의 방향을 기억한다. 그 |j|가 양수면 뒤집힌 사본을
거꾸로 훑으며
...
@ 타일에는 {\it 형\/}(type)이 있다. 형은 $0$부터 $3$까지 넷인데, 타일에 $i$를 곱하면
...
@ 타일의 접기 곱은 지금 경로의 $z$에 매여 있다. 이제 $d$를 $w$의 형이라 할 때
$$v*w=i^{-d}v+z\,(w+i^{2-d})$$
...
@ 명령 \.{a*}는 폴리오미노의 타일 모두에 주어진 타일을 곱한다. 이를테면
\.{a*1,0}은 단위 타일을 곱하는 것이니 폴리오미노를 그냥 늘어놓는 셈이다.
...
이제 $Z=(2+2i)z$라 하자. 두 타일이 {\it 합동\/}이라 함은 하나에 $i$의 거듭제곱을 곱하고
$Z$의 배수를 더해 다른 하나로 갈 수 있다는 뜻이다. 그러니 합동류를 셈하려면 타일을
$Z$로 나눈 나머지로 줄인 다음, $i$를 곱해 만나는 넷을 한 무리로 묶으면 된다.

수 $Z$의 배수들이 이루는 격자를 다루기 좋은 기저로 바꾸는 것이 첫 일이다. 두 수
$Z$와 $iZ$가 그 격자를 낳는데, 이 둘에 정수 조합을 거듭 먹여 $(U,0)$과 $(V_x,D)$ 꼴로 만든다.
...
값이 $z=0$이면 $Z=0$이라 나눌 것이 없다. 원본은 이 경우를 살피지 않아 $0$으로 나누는데,
기계에 따라 죽거나(x86) 엉뚱한 값을 뱉는다(ARM에서 실제로 그랬다). \GO/는 $0$으로
나누면 반드시 패닉이므로 미리 막는다. 명령 \.{p0123}처럼 제자리로 돌아오는 경로가
그런 경우다.
...
홀짝이 어긋난 값을 주면 엉뚱한 칸을 읽고 엉뚱한 번호를 답한다. 함수 |tile|이 이미 살피므로
여기서는 그럴 일이 없다.
...
@ 쪼개는 식은 곱셈 식을 뒤집은 것이다. 타일 $u$와 합동인 폴리오미노의 타일 $v$를 찾고,
...
@ |cyc[0]|에는 지금까지 본 것 가운데 가장 작은 것을 둔다. 크기가 $\vert w\vert=1$이면
